...
// ============================================
// TTS COMPARISON TEST — admin only
// ============================================

const TTS_WORDS = [
  { kanji: '猫',      hiragana: 'ねこ',           english: 'cat (2 mora)' },
  { kanji: '冒険',    hiragana: 'ぼうけん',        english: 'adventure (4 mora)' },
  { kanji: '食べ物',  hiragana: 'たべもの',        english: 'food (4 mora)' },
  { kanji: '図書館',  hiragana: 'としょかん',      english: 'library (5 mora)' },
  { kanji: '気をつけてください', hiragana: 'きをつけてください', english: 'please be careful (10 mora)' },
];
...
const TTSComparison = () => {
  const [status, setStatus] = useState({});  // key → 'loading' | 'ready' | 'error'
  const [playing, setPlaying] = useState(null); // key of currently playing button
  const [timings, setTimings] = useState({});   // key → ms string
  const cacheRef = useRef({});  // key → data URL
  ...
  const fetchTTS = useCallback(async (text, hiragana, speed) => {
    const t0 = performance.now();
    const data = await callCloudFunction('synthesizeSpeech', { text, hiraganaHint: hiragana, speed });
    const elapsed = Math.round(performance.now() - t0);
    return { dataUrl: 'data:audio/mp3;base64,' + data.audioContent, elapsed };
  }, []);

  const playClip = useCallback((dataUrl, playbackRate, key) => {
    stopCurrent();
  ...
  // Fetch a specific clip and cache it
  const ensureClip = useCallback(async (wi, speed, cacheKey) => {
    if (cacheRef.current[cacheKey]) return cacheRef.current[cacheKey];
    setStatus(s => ({ ...s, [cacheKey]: 'loading' }));
    try {
      const word = TTS_WORDS[wi];
      const result = await fetchTTS(word.kanji, word.hiragana, speed);
      cacheRef.current[cacheKey] = result.dataUrl;
      setTimings(t => ({ ...t, [cacheKey]: `${result.elapsed}ms` }));
      setStatus(s => ({ ...s, [cacheKey]: 'ready' }));
      return result.dataUrl;
    } catch (e) {
      setStatus(s => ({ ...s, [cacheKey]: 'error' }));
      return null;
    }
  }, [fetchTTS]);

  // Column A: server-generated at each speed
  const playServerSpeed = useCallback(async (wi, si) => {
    const key = `A_${wi}_${si}`;
    const dataUrl = await ensureClip(wi, TTS_SPEEDS[si].rate, key);
    if (dataUrl) playClip(dataUrl, 1.0, key);
  }, [ensureClip, playClip]);

  // Column B: normal-speed clip + client playbackRate
  const playClientRate = useCallback(async (wi, si) => {
    const normalKey = `B_${wi}`;
    const dataUrl = await ensureClip(wi, 1.0, normalKey);
    if (dataUrl) playClip(dataUrl, TTS_SPEEDS[si].rate, `B_${wi}_${si}`);
  }, [ensureClip, playClip]);

  const btnStyle = (key) => ({
    background: playing === key ? '#ffe600' : 'rgba(0,0,0,0.04)',
    color: playing === key ? '#1a1a2e' : '#333',
    border: 'none', padding: '5px 10px', borderRadius: '6px',
    fontSize: '12px', cursor: 'pointer', minWidth: '60px',
  });

  const timingStyle = { fontSize: '10px', color: '#aaa', textAlign: 'center', minHeight: '14px', marginBottom: '4px' };

  return (
    <>
      <div style={{ fontSize: '11px', fontWeight: '700', color: '#5f6368', textTransform: 'uppercase', letterSpacing: '0.5px', marginTop: '20px', marginBottom: '8px', paddingLeft: '4px' }}>TTS Comparison Test</div>
      <div style={{ background: '#fff', borderRadius: '14px', overflow: 'hidden', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '12px 14px' }}>
        <div style={{ fontSize: '11px', color: '#888', marginBottom: '12px', lineHeight: 1.5 }}>
          <b>A:</b> Google TTS generates at each speed (current). <b>B:</b> One normal clip + browser playbackRate (proposed). Tap to fetch & play.
        </div>

        {TTS_WORDS.map((word, wi) => (
          <div key={wi} style={{ borderTop: wi > 0 ? '1px solid #f0f0f0' : 'none', paddingTop: wi > 0 ? '10px' : 0, marginBottom: '10px' }}>
            <div style={{ display: 'flex', alignItems: 'baseline', gap: '8px', marginBottom: '8px' }}>
              <span style={{ fontSize: '20px', fontFamily: "'Noto Serif JP', serif" }}>{word.kanji}</span>
              <span style={{ fontSize: '12px', color: '#888' }}>{word.hiragana}</span>
              <span style={{ fontSize: '11px', color: '#bbb' }}>{word.english}</span>
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '8px' }}>
              <div style={{ borderRight: '1px solid #f0f0f0', paddingRight: '8px' }}>
                <div style={{ fontSize: '10px', fontWeight: '700', color: '#aaa', textAlign: 'center', marginBottom: '4px' }}>A: SERVER SPEED</div>
                {TTS_SPEEDS.map((s, si) => {
                  const key = `A_${wi}_${si}`;
                  return (
                    <div key={si} style={{ textAlign: 'center', marginBottom: '2px' }}>
                      <button style={btnStyle(key)} onClick={() => playServerSpeed(wi, si)}>
                        {status[key] === 'loading' ? '...' : s.label}
                      </button>
                      <div style={timingStyle}>{timings[key] || ''}</div>
                    </div>
                  );
                })}
              </div>
              <div>
                <div style={{ fontSize: '10px', fontWeight: '700', color: '#aaa', textAlign: 'center', marginBottom: '4px' }}>B: CLIENT RATE</div>
                {TTS_SPEEDS.map((s, si) => {
                  const key = `B_${wi}_${si}`;
                  const normalKey = `B_${wi}`;
                  return (
                    <div key={si} style={{ textAlign: 'center', marginBottom: '2px' }}>
                      <button style={btnStyle(key)} onClick={() => playClientRate(wi, si)}>
                        {status[normalKey] === 'loading' && !cacheRef.current[normalKey] ? '...' : s.label}
                      </button>
                      <div style={timingStyle}>
                        {si === 0 ? (timings[normalKey] || '') : (cacheRef.current[normalKey] ? `rate=${s.rate}` : '')}
                      </div>
                    </div>
                  );
                })}
  ...
  return (
    <div style={{
      ...
      <div style={{ maxWidth: '430px', margin: '0 auto' }}>
        ...
        <div style={{ fontSize: '11px', color: '#bbb', marginBottom: '16px' }}>Kanji Hunt v3.3.5 · Hatake Development</div>
